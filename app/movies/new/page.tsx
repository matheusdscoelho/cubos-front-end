...
const schema = z.object({
  ...
  duration: z.number({ message: 'Duração em minutos' }).min(1, 'Duração em minutos'),
  budget: z
    .number({ message: 'Orçamento deve ser positivo' })
...
export default function NewMoviePage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      const errorMsg = (err as ErrorResponse)?.response?.data?.error || 'Erro ao cadastrar filme';
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      <h1 className="text-2xl font-bold mb-6 text-gray-800 dark:text-white">Novo Filme</h1>
